...
{
    let creds = connection.peer_credentials().await.map_err(|e| {
    ...
    let app_id = app_id_from_pid(creds.process_id().as_raw_pid() as u32)
        .await
...
pub fn validate_serialized_icon(
...
) -> Result<(String, String, glib::Bytes), anyhow::Error> {
    let string = icon_v
        .as_str()
    ...
    Ok((icon_format, icon_size, bytes_icon.bytes()))
}
